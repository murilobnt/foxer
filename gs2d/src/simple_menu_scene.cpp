...
SimpleMenuScene::SimpleMenuScene(std::string background_texture_path, int w,
                                 int h) {
  TextureLoader::set_texture_from_file(background_texture,
                                       background_texture_path, w, h);
}
...
void SimpleMenuScene::update() { delayed_update(); }
...
void SimpleMenuScene::add_text(sf::Text &text) { ui_text_vec.push_back(text); }
