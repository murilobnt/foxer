...
TiledLevel::TiledLevel(const std::string& tileset, const std::string &json_tiled_file){
    load(tileset, json_tiled_file);
}

void TiledLevel::load(const std::string& tileset, const std::string &json_tiled_file){
    std::ifstream my_json;
    my_json.open(json_tiled_file);
    json level = json::parse(my_json);
    my_json.close();

    tile_size = sf::Vector2u(level["tilewidth"].get<int>(), level["tileheight"].get<int>());
    level_size = sf::Vector2u(level["width"].get<int>(), level["height"].get<int>());
    ...
    for(json::iterator it = layers.begin(); it != layers.end(); ++it){
        if((*it)["type"] == "tilelayer"){
             ...
             for(json::iterator it2 = (*it)["objects"].begin(); it2 != (*it)["objects"].end(); ++it2){
